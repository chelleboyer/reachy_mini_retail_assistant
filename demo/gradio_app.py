...
sys.path.append(str(Path(__file__).parent.parent / "reachy_edge"))

from cache.l2_cache import ThreadSafeProductCache
from models import Product

# Try to import OpenAI for real LLM responses
try:
    import openai
...
# Initialize thread-safe cache (each Gradio request runs in different thread)
cache = ThreadSafeProductCache(db_path="./data/cache.db")
cache.initialize()

# Interaction history for telemetry
interaction_history = []

# Mock LLM responses (replace with real LLM API later)
RESPONSE_TEMPLATES = {
...
def get_llm_response(query: str, results: list) -> str:
    ...
    try:
        ...
        if results:
            products_context = "I found these products:\n"
            for i, p in enumerate(results[:3], 1):
                products_context += f"{i}. {p.name} - ${p.price:.2f} at {p.location}\n"
        else:
            products_context = "I searched but didn't find any matching products in our inventory."
        ...
        response = openai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system", 
                    "content": "You are Reachy, a helpful and friendly truck stop retail assistant. You help truckers find products. Keep responses brief (2-3 sentences), conversational, and helpful. If products are found, mention the top result. If not found, politely say we don't carry it and suggest alternatives if possible."
                },
                {
                    "role": "user", 
                    "content": f"Customer asks: '{query}'\n\n{products_context}\n\nRespond naturally:"
                }
            ],
            max_tokens=150,
            temperature=0.7
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    **Interaction Loop Demonstration:**
    ...
    - **GPT-4o-mini** generates conversational responses (or mock if no API key)
    ...
    **💡 Tip:** Set `OPENAI_API_KEY` environment variable for real GPT responses!
    """)
