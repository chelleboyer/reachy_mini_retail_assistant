"""Event emitter for sending events to the Second Brain backend."""
import asyncio
import httpx
import logging
...
class EventEmitter:
    """Batched event emitter to the Second Brain backend."""
    ...
    def __init__(
        ...
        self.backend_api_key = backend_api_key or settings.backend_api_key
        ...
        self.enabled = enabled if enabled is not None else settings.backend_enabled
        ...
        if not self.enabled:
            logger.info("Second Brain client disabled (backend_enabled=False)")
    
    async def emit(self, event_data: Dict[str, Any]) -> None:
        """Queue an event for batched sending."""
        if not self.enabled:
            logger.debug("Event not emitted (Second Brain client disabled)")
            return
    ...
    async def worker(self) -> None:
        ...
        if not self.enabled:
            logger.info("Event worker not started (Second Brain client disabled)")
            return
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.post(
                    f"{self.backend_url}/events/batch",
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Sent {len(batch_to_send)} events to backend")
                else:
            ...
            self._events_failed += len(batch_to_send)
            logger.error(f"Error sending events to backend: {e}")
